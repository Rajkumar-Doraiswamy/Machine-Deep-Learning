...
ed.isnull()
(ed.isnull()).apply(sum, axis = 0)            # total_toilets: 338, establishment_year: 338


# 8.2  Imputing data. Strategy can be difft for each column
#      For total_toilets apply 'mean'
imp = SimpleImputer(strategy="mean")     # Could have been median also
...
6. ADASYN (from Wikipedia)
    ...
    on the methodology of SMOTE (Synthetic Minority Over Sampling), by 
    shifting the importance of the classification boundary to those
    minority classes which are difficult. ADASYN uses a weighted 
    distribution for different minority class examples according to their 
    level of difficulty in learning, where more synthetic data is
    generated for minority class examples that are harder to learn.   
